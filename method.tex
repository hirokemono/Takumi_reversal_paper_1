...
% for incompressible fluid given as
%
\begin{equation}
\nabla \cdot \bvec{u} = 0,
\label{eq:divu=0}
...
The magnetic field, $\bvec{B}$, satisfies the Gauss's law % of which differential form is 
given as
%
\begin{equation}
\nabla \cdot \bvec{B} = 0.
...
\label{eq:BT_BS}
\end{equation}
where % $\bvec{r}$ is the position vector, and 
...
An uniform temperature gradient at the CMB is imposed as $\beta_o = -\partial T_0^0 / \partial r |_{r = r_o} = 0.4225$. 
...
Consequently, the uniform temperature gradient at the ICB is set to $-\partial T_0^0 / \partial r |_{r = r_i} = (r_o/r_i)^2 \beta_o$.
...
$-\bvec{u}^s \cdot (\bvec{J}^a \times \bvec{B}^s)$ to those of $\bvec{B}^s$ by % $\bvec{u}^a$, 
{\color{red} $\bvec{u}^s$,}
